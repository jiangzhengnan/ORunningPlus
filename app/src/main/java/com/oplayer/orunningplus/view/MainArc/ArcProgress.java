...
public class ArcProgress extends ProgressBar {
    ...
    private final int DEFAULT_LINEHEIGHT = dp2px(7);
    private final int DEFAULT_mTickWidth = dp2px(1);
    private final int DEFAULT_mRadius = dp2px(36);
...
//    private final int DEFAULT_OFFSETDEGREE = 60;
    private final int DEFAULT_OFFSETDEGREE = 20;
    private final int DEFAULT_DENSITY = 6;
    ...
    public ArcProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        currStep = attributes.getFloat(R.styleable.ArcProgress_currStep, 0);
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int AnserW = getLocalResolution(mBoardWidth);
        int AnserR = getLocalResolution(mRadius);
        if (widthMode != MeasureSpec.EXACTLY) {
            int widthSize = (int) (AnserR * 2 + AnserW * 2);
            widthMeasureSpec = MeasureSpec.makeMeasureSpec(widthSize, MeasureSpec.EXACTLY);
        }
        if (heightMode != MeasureSpec.EXACTLY) {
            int heightSize = (int) (AnserR * 2 + AnserW * 2);
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(heightSize, MeasureSpec.EXACTLY);
        }
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
    }

    public int getLocalResolution(float radius) {
        int ScreenWidth = getScreenWidth();
        double screen = (double) 1080 / (double) ScreenWidth;
        return (int) (radius / screen);
    }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        mArcPaint.setStrokeWidth(AnserW);
        mArcPaint.setStyle(Paint.Style.STROKE);
        mLinePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mLinePaint.setStrokeWidth(mTickWidth);
        canvas.save();
        float roate = getProgress() * 1.0f / getMax();
        float x = mArcRectf.right / 2 + AnserW / 2;
        float y = mArcRectf.right / 2 + AnserW / 2;
        if (mOnCenter != null) {
            if (mCenterCanvas == null) {
                mCenterBitmap = Bitmap.createBitmap((int) (AnserR * 2), (int) (AnserR * 2), Bitmap.Config.ARGB_8888);
                mCenterCanvas = new Canvas(mCenterBitmap);
            }
            mCenterCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
            mOnCenter.draw(mCenterCanvas, mArcRectf, x, y, AnserW, getProgress());
        ...
        if (mStylePogress == STYLE_ARC) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                canvas.drawLine(x, AnserW + AnserW / 2, x, AnserW - AnserW / 2, mLinePaint);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int AnserW = getLocalResolution(mBoardWidth);
        int AnserR = getLocalResolution(mRadius);
        mArcRectf = new RectF(AnserW, AnserW, AnserR * 2 + AnserW, AnserR * 2 + AnserW);
    }
    ...
    //设置弧形粗细
    public ArcProgress setBoardWidth(int size) {
        if (size > 0)
            mBoardWidth = size;
        return this;
    }
    ...
    //设置弧形半径
    public ArcProgress setArc_Radius(int size) {
        if (size > 0)
            mRadius = size;
        return this;
    }
    ...
    //设置弧形颜色
    public ArcProgress setProgressColor(int color) {
